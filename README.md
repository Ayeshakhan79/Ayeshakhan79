<h1>Hi, I'm Ayesha Khan! <br/><a href="https://github.com/ayeshakhan-data">Data Analyst / Data Scientist</a>, <a href="https://www.linkedin.com/in/ayesha-khan/"></a></h1>

<h2>💻 Technologies & Tools I Work With:</h2>

- <b>Programming & Scripting:</b> Python, R  
- <b>Data Analytics & Visualization:</b> Power BI, Excel, Tableau  
- <b>Data Processing & ETL:</b> Apache Airflow, Pandas, NumPy  
- <b>Machine Learning & AI:</b> Scikit-learn, TensorFlow, Keras  
- <b>Databases & Platforms:</b> PostgreSQL, MySQL, Microsoft SQL Server, SharePoint  
- <b>Cloud:</b> Azure, AWS  
- <b>Power Platform & Apps:</b> Power Platform, Power Apps, SharePoint Apps, Power Automate, Streamlit  
- <b>Microsoft Copilot Studio:</b> Created agents and automated workflows


<h2> 🤳 Connect with me:</h2>

[<img align="left" alt="Ayesha Khan | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]
[<img align="left" alt="Ayesha Khan | GitHub" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/github.svg" />][github]

<br/>

[linkedin]: https://linkedin.com/in/ayesha-khan
[github]: https://github.com/Ayeshakhan79
